<script setup lang="ts">
    ...
    function getLabel(it: string | { label: string; value: string }) {
        return typeof it === 'string' ? it : it.label
    }
    function getValue(it: string | { label: string; value: string }) {
    ...
    function scroll(dir: 'left' | 'right') {
        ...
        if (!props.centerOnSelect && el) {
            const delta =
                dir === 'left' ? -(props.scrollAmount ?? 240) : (props.scrollAmount ?? 240)
    ...
    onMounted(() => {
        ...
        const onWheel = (e: WheelEvent) => {
            ...
            if (delta) {
                ...
                el.scrollBy({
                    left: delta,
                    behavior: props.smoothScroll === false ? 'auto' : 'smooth',
                })
                schedule()
            }
        }
...
<template>
    <div
        class="z-scroll-chips"
        :style="
            props.stickyTop != null
                ? {
                      position: 'sticky',
    ...
    >
        <button
            class="z-scroll-chips__btn"
            :data-show="canLeft ? 'true' : 'false'"
            @click="scroll('left')"
        >
            <ZIcon
                :name="
                    (props.markerDirection ?? 'lr') === 'ud' ? 'bx:chevron-up' : 'bx:chevron-left'
                "
            />
        </button>
        <div
            class="z-scroll-chips__scroll"
            :class="{ 'is-snap': props.snapCenter && !isCentering }"
            ref="setsEl"
        >
            <div class="z-scroll-chips__items">
                <ZButton
                    ...
                    :variant="
                        getValue(it) === selected
                            ? (props.activeVariant ?? 'primary')
                            : (props.inactiveVariant ?? 'default')
                    "
