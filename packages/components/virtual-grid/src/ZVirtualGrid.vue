...
const visibleRange = computed(() => {
  const startRow = Math.max(0, Math.floor(scrollTop.value / rowHeight.value) - bufferRows.value)
  ...
  <div class="z-virtual-grid" ref="root">
    <div class="z-virtual-grid__content" :style="{ height: contentH + 'px' }">
      <div
        ...
        :style="{ transform: `translate(${c.x}px, ${c.y}px)`, width: cellWidth + 'px', height: props.itemHeight + 'px' }"
      >
        <slot name="cell" :item="c.item" :index="c.i"></slot>
      </div>
    </div>
    <div v-if="props.showMarkers !== false" class="z-virtual-grid__markers">
      <button class="z-virtual-grid__marker" :data-show="canUp ? 'true' : 'false'" @click="scrollBy('up')">
        <ZIcon name="bx:chevron-up" />
      </button>
      <button class="z-virtual-grid__marker" :data-show="canDown ? 'true' : 'false'" @click="scrollBy('down')">
