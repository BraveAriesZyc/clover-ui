...
- 样式文件：`packages/theme-chalk/src/Z<Name>.css`（在 `packages/theme-chalk/index.css` 引入）
...
样式规范：优先使用 CSS 变量（如 `--color-bg`、`--color-text`、`--color-border`）。构建后会自动复制到 `dist/library/theme-chalk/Z<Name>.css`，供按需样式引入。
...
这样即可在模板中直接使用 `<ZName />` 获得类型提示。类型系统已同时兼容根入口与子路径导出。
