...
function goDocs() { router.push('/docs/virtual-grid') }

const items = computed(() => Array.from({ length: 2000 }, (_, i) => i + 1))
</script>

<template>
  <div class="section">
    <div class="section__title">ZVirtualGrid 虚拟网格 <ZButton size="sm" variant="default" @click="goDocs">文档</ZButton></div>
    <div class="gallery">
...
.cell { display: flex; align-items: center; justify-content: center; height: 64px; border: 1px solid var(--color-border); border-radius: 8px; background: var(--color-bg); color: var(--color-muted); }
</style>
